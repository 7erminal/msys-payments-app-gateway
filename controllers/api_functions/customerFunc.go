...
func GetCustomerDetailsWithPhoneNumber(c *beego.Controller, phoneNumber string) (resp responses.CustomerResponseDTO) {
	host, _ := beego.AppConfig.String("customerBaseUrl")

	logs.Info("Getting user details ", phoneNumber)

	request := api.NewRequest(
		host,
		"/v1/customers/phone-number/"+phoneNumber,
		api.GET)
	// request.Params["username"] = username
	// request.Params = {"UserId": strconv.Itoa(int(userid))}
	client := api.Client{
...
func GetUserDetailsWithCode(c *beego.Controller, userCode string) (resp responses.UsersResponseDTO) {
	host, _ := beego.AppConfig.String("customerBaseUrl")

	logs.Info("Getting user details ", userCode)

	request := api.NewRequest(
		host,
		"/v1/users/branch/"+userCode,
		api.GET)
	// request.Params["username"] = username
	// request.Params = {"UserId": strconv.Itoa(int(userid))}
	client := api.Client{
